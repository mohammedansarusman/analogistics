...
const EmployeeList = () => {
  ...
  useEmployeeData();
  const employeeData = useSelector((store)=>store.registration.employeeRecords);
  const filteredData = useSelector((store)=>store.registration.filterEmployeeRecords)
  ...
  return (
    <div className="w-full absolute left-0">
      ...
      <div className="w-full flex justify-center items-center px-5 md:gap-4 lg:justify-end lg:gap-5  lg:items-center py-2">
        ...
        <div className="hidden text-gray-500 text-xl cursor-pointer p-2 
                        md:block 
                        lg:block
        ...
        <div className="text-gray-500 text-xl cursor-pointer p-2
                        hover:font-bold hover:bg-gray-500 hover:p-2 hover:text-white hover:rounded-full
                        transition-all duration-500 ease-in-out">
          <FaSortAmountUp />
        </div>
      ...
      <div className={`flex  justify-center items-center bg-red-200 ${display === 'list' ? "flex-col" : "flex-wrap"}`}>
          {
            filteredData.map((emp) => {
              return (
                  <div 
                    className={`flex flex-col w-full items-center ${ display === "grid" ? "md:w-[40%] lg:w-[30%]" : "md:w-[80%]"}`} 
                    key={emp.firebaseId}>
